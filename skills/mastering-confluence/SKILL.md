...
python3 scripts/safe_update_confluence.py fragment.md --page-id PAGE_ID --insert-at top
```

MCP tools are fine for **reading** pages but fail for uploading large content.

**For existing page edits, do not re-render the full page from Markdown.**
Convert only the user supplied edit text to Confluence storage format, then patch
that storage fragment into the original Confluence storage body at the exact
requested insert or replace location. The unchanged storage text must remain
unchanged.

## Quick Start

### Safe Storage-Format Edits

```bash
# Insert converted Markdown at the top of an existing page
python3 scripts/safe_update_confluence.py fragment.md --page-id 780369923 --insert-at top

# Insert after an exact storage marker
python3 scripts/safe_update_confluence.py fragment.md --page-id 780369923 --insert-after @marker.storage

# Replace an exact storage selection
python3 scripts/safe_update_confluence.py fragment.md --page-id 780369923 --replace-selection @old-section.storage
```

### Create A Page

```bash
# Convert Markdown to storage format, then create the page
python3 scripts/safe_update_confluence.py document.md --space DEV --title "Page title" --parent-id 123456
```
...
|------------|-------------|-----------|
| Safe storage-format page edits/creates | `safe_update_confluence.py` | [upload_guide](references/upload_guide.md) |
| Whole-page Markdown replacement | `upload_confluence_v2.py` | [upload_guide](references/upload_guide.md) |
...
- [ ] User supplied edit text converted to storage format before upload
- [ ] Existing page edits use exact `--insert-*` or `--replace-selection` location
- [ ] Dry-run storage diff shows only the requested section changed
- [ ] Dry-run tested: `--dry-run`
- [ ] Upload executed with safe wrapper or v2 script (not MCP)
...
3. **Upload** (safe wrapper handles storage conversion before upload):
   ```bash
   python3 scripts/safe_update_confluence.py fragment.md --page-id PAGE_ID --insert-at top
   ```
...
| Script | Purpose |
|--------|---------|
| `safe_update_confluence.py` | Convert supplied text to storage format, patch exact storage sections, and create/update pages safely |
